...
def validate_audio_input(file_path: Path) -> None:
    ...
    if file_size_mb > MAX_AUDIO_FILE_SIZE_MB:
        raise ValidationError(f"Audio file too large: {file_size_mb:.2f} MB (max: {MAX_AUDIO_FILE_SIZE_MB} MB)")
...
def validate_image_input(file_path: Path) -> None:
    ...
    if file_size_mb > MAX_IMAGE_FILE_SIZE_MB:
        raise ValidationError(f"Image file too large: {file_size_mb:.2f} MB (max: {MAX_IMAGE_FILE_SIZE_MB} MB)")
